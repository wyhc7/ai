...
典型症状：**短回答正常，长回答输出到一半断掉**，而直连上游官方 API 时没有这个问题。

一次流式请求能忍受多久没有数据，取决于链路上**最短**的那个超时：

```
客户端 → [隧道 / 反向代理，可选] → ai-gateway → 上游官方 API
```

任何一环先断开，客户端看到的现象都一样：输出戛然而止。所以先定位是哪一环。

### 第 1 步：看日志里写了什么

管理界面 → 日志页面，找到断流那条记录，看红色错误文字：

| 日志显示 | 说明 |
| ---- | ---- |
| `上游长时间无数据（空闲超时，已断开）` | 网关等上游太久，可加大 `STREAM_IDLE_TIMEOUT_MS` |
| `流式转发中断: ...` | 上游或网络主动断开，检查服务器到上游的链路 |
| `上游请求超时或长时间无响应` | 撞上了连接或总时长超时 |
| **状态 200、没有任何错误** | 网关认为一切正常，是网关之前的链路（客户端 ↔ 网关）断了 |

最后一行最容易被忽略，也最常见：网关把内容完整发出去了，但中间设备没能把它送完。

### 第 2 步：在服务器本机对照测试

绕过公网链路，直接请求本机的 3001 端口：

```bash
curl -N -H "Authorization: Bearer <网关Key>" \
  -H "Content-Type: application/json" \
  -d '{"model":"你的模型","stream":true,"messages":[{"role":"user","content":"请写一篇很长的文章"}]}' \
  http://127.0.0.1:3001/api/v1/chat/completions
```

- **本机正常、公网访问断** → 问题在客户端到网关之间的链路，看第 3 步
- **本机同样断** → 问题在网关或上游，看第 1 步的日志结论

### 第 3 步：按访问方式处理

**nginx 反向代理**（本项目的推荐配置已包含以下两项，若自己写的配置请核对）：

```nginx
proxy_buffering off;      # 关键：开启缓冲会把流式响应攒着，长回答被延迟甚至截断
proxy_read_timeout 3600s; # 两次读取之间的间隔，不是总时长
```

**Caddy** 默认不缓冲响应，通常无需额外配置；若加了 `reverse_proxy` 的 `flush_interval`，设为 `-1`。

**Cloudflare**：域名的代理状态为「已代理」（橙色云）时，Cloudflare 会在约 100 秒无数据传输后返回 524。
流式输出期间一直有数据通常不会触发，但**上游思考阶段较长时**会。可选做法：
把该域名改为「仅 DNS」（灰色云）直连，或改用 Cloudflare Tunnel。

**frp / 其他内网穿透**：检查服务端的空闲超时与单连接时长限制，长回答场景建议放宽。

### 可用环境变量

网关侧的超时均可通过环境变量调整，无需改代码：

| 变量 | 默认值 | 说明 |
| ---- | ------ | ---- |
| `CONNECT_TIMEOUT_MS` | `30000` | 连接与等待响应头的超时 |
| `STREAM_IDLE_TIMEOUT_MS` | `600000` | 流式响应多久没数据就判定上游已死。推理模型思考时间长时应加大 |
| `STREAM_TOTAL_TIMEOUT_MS` | `1800000` | 单次流式请求的总时长上限 |
| `JSON_TOTAL_TIMEOUT_MS` | `120000` | 单次非流式请求的总时长上限 |
| `SSE_HEARTBEAT_INTERVAL_MS` | `15000` | 心跳间隔。必须小于链路上最短空闲超时的一半 |
| `BACKPRESSURE_MAX_WAIT_MS` | `60000` | 客户端不消费时最多等多久，超时后放弃等待以免拖垮上游 |

网关对流式响应会自动加上 `X-Accel-Buffering: no` 与 `Cache-Control: no-cache, no-transform`，
用于关闭 nginx 等中间层的缓冲，通常无需在网关之外再做什么。
